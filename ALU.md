...
En el pipeline de 4 etapas (FETCH | DECODE | EXEC | WB), la ALU opera en la etapa **EXEC** de forma combinacional: la Unidad de Control activa `ALU_Op` y las señales de escritura en el mismo ciclo en que los operandos están disponibles en el banco de registros.
...
| `processor/ALU_pkg.vhdl` | Package con las constantes de opcodes y tipos |
...
entity ALU is
    Port (
        RegInA    : in  data_vector;        -- Operando A: R0 (Acumulador)
        RegInB    : in  data_vector;        -- Operando B: R0..R7 vía Mux
        Oper      : in  opcode_vector;      -- Código de operación (5 bits)
...
| :---: | :----: | :--- | :--- |
| 7 | `C` | Carry/Borrow | Desbordamiento sin signo en sumas; préstamo en restas (`NOT borrow`, convención ARM). Para `MUL`/`MUH`: `1` si el producto > 255. |
...
| 2 | `E` | Equal | `A = B` (comparación bit a bit). Calculado en **todas** las operaciones. |
| 1 | `R` | Bit desplazado (right) | Bit 0 de A antes del desplazamiento en `LSR` y `ASR`. |
| 0 | `L` | Bit desplazado (left) | Bit 7 de A antes del desplazamiento en `LSL` y `ASL`. |
...
| Mnemónico | Opcode (bin) | Opcode (hex) | Operación | Flags adicionales |
| :---: | :---: | :---: | :--- | :--- |
| `NOP` | `00000` | `0x00` | Sin operación; G y E sí se actualizan | — |
...
En operaciones de resta (`SUB`, `SBB`, `DEC`, `CMP`, `NEG`), el flag **C** sigue la convención:

- `C = 1` → no hubo borrow (resultado ≥ 0 sin signo extendido a 9 bits)
- `C = 0` → hubo borrow (resultado < 0, underflow sin signo)
